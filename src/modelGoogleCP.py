...
def Hybridflowshopmodel(instance, mdl):
    ...
    for j in range(instance.n):
        for i in range(instance.g):
            mdl.Add(
                sum([all_tasks[j, i, k].is_present for k in range(instance.m[i])]) == 1
            )
    ...
    mdl.AddMaxEquality(
        C_max,
        [
            all_tasks[j, instance.g - 1, k].end
            for j in range(instance.n)
            for k in range(instance.m[instance.g - 1])
        ],
    )
    mdl.Minimize(C_max)
...
def openshopmodel(instance, mdl):
    ...
    mdl.AddMaxEquality(
        C_max,
        [all_tasks[j, i].end for j in range(instance.n) for i in range(instance.g)],
    )
    mdl.Minimize(C_max)
...
def jobshopmodel(instance, mdl):
    ...
    for j in range(instance.n):
        jobs_data[j] = [
            (instance.r[j][i] - 1, instance.p[j][i]) for i in range(instance.g)
        ]
    ...
    C_max = mdl.NewIntVar(0, horizon, "C_max")
    mdl.AddMaxEquality(
        C_max, [all_tasks[j, len(job) - 1].end for j, job in enumerate(jobs_data)]
    )
    mdl.Minimize(C_max)
