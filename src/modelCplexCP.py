...
def TCTflowshopmodel(instance, mdl):
    ...
    mdl.add(
        mdl.minimize(
            mdl.sum([mdl.end_of(tasks[j][instance.g - 1]) for j in range(instance.n)])
        )
...
def Distributedflowshopmodel(instance, mdl):
    ...
    for j in range(instance.n):
        _tasks[j] = [
            mdl.interval_var(name="T_{}_{}".format(j, i)) for i in range(instance.g)
        ]
    for j in range(instance.n):
        for i in range(instance.g):
            # if i == 0:
    ...
    for i in range(instance.g - 1):
        for k in range(instance.f):
            mdl.add(
                mdl.same_sequence(Sequence_variable[i][k], Sequence_variable[i + 1][k])
            )
    ...
    mdl.add(
        mdl.minimize(
            mdl.max([mdl.end_of(_tasks[j][instance.g - 1]) for j in range(instance.n)])
        )
    )  # this is makespan
...
def Setupflowshopmodel(instance, mdl):
    ...
    mdl.add(
        mdl.minimize(
            mdl.max([mdl.end_of(tasks[j][instance.g - 1]) for j in range(instance.n)])
        )
    )  # this is makespan
...
def Flexiblejobshopmodel(instance, mdl):
    ...
    for j in range(instance.n):
        _tasks[j] = [
            mdl.interval_var(name="T_{}_{}".format(j, k)) for k in range(instance.o[j])
        ]
    for j in range(instance.n):
        for k in range(instance.o[j]):
            mdl.add(
    ...
    mdl.add(
        mdl.minimize(
            mdl.max(
                [mdl.end_of(_tasks[j][instance.o[j] - 1]) for j in range(instance.n)]
            )
        )
    )  # this is makespan
...
def Tardinessflowshopmodel(instance, mdl):
    ...
    mdl.add(
        mdl.minimize(
            mdl.sum(
                [
                    mdl.max([mdl.end_of(tasks[j][instance.g - 1]) - instance.d[j], 0])
                    for j in range(instance.n)
                ]
            )
        )
        ...
        for i in range(1, instance.g):
            mdl.add(
                mdl.end_before_start(
                    tasks[j][instance.r[j][i - 1] - 1], tasks[j][instance.r[j][i] - 1]
                )
            )  # no overlap jobs
...
def prallelmachinemodel(instance, mdl):
    ...
    makespan = max(
        [
            sum([instance.p[j][i] * (machine[j] == i) for j in range(instance.n)])
            for i in range(instance.g)
        ]
    )
    mdl.add(
